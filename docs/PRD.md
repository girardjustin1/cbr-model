...
- **13 Tom ↔ Python parity gate: NOT READY** (checklist `docs/governance/phase13-readiness.md`, 15/22 items; protocol v0.2 approved in principle; parity-candidate specs `docs/strategy/parity-candidates/*-PC1.yaml` frozen; parity library `src/cbr/engine/parity.py` refuses the final run until V-1, calibration and frozen tolerances; open owner items OQ-44, OQ-45). The engine must reproduce the taught examples before any performance research. **V-1 hard requirement (D16-3):** tick-mid structure compared with a FOREXCOM:XAUUSD export on the course-example windows; material differences reopen OQ-25
...
**Phase 10 accepted (D15, G10: `docs/governance/g10-approval.md`). OQ-25 resolved (D16): STRUCTURE = tick mid, EXECUTION = tick bid/ask; guards in `src/cbr/data/price_series.py`. Phase 11 accepted (D17, G11); CBR15 not baseline-eligible (OQ-36 evidence package, hourly veto). Phase 12 accepted (D18, G12). D19 rulings implemented; PC1 parity-candidate specs frozen. Phase 13 NOT READY: V-1 export, calibration, frozen tolerances, OQ-44/45 and final-run authorization outstanding.** Full tick history deferred (OQ-33); V-1 required before Phase 13. Phase 14 is
...
| OQ-44 / OQ-45 | HVCS indecision continuity reading; CBR15 condition-window basis | Owner decisions (raised in D19 implementation) |
