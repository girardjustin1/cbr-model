...
- **13 Tom ↔ Python parity gate: NOT READY** (checklist `docs/governance/phase13-readiness.md`, 21/28 items; parity-candidate specs `docs/strategy/parity-candidates/*-PC2.yaml` frozen, PC1 kept; parity library refuses the final run; V-1 state **V1_TOM_DATA_INSUFFICIENT** (FOREXCOM source present, 1m history too short; Dukascopy side ready), supplied charts don't satisfy V-1: `reports/v1-export-inspection.md`). The engine must reproduce the taught examples before any performance research.
...
**Phase 10 accepted (D15, G10: `docs/governance/g10-approval.md`). OQ-25 resolved (D16): STRUCTURE = tick mid, EXECUTION = tick bid/ask; guards in `src/cbr/data/price_series.py`. Phase 11 accepted (D17, G11); CBR15 not baseline-eligible (OQ-36 evidence package, hourly veto). Phase 12 accepted (D18, G12). D19 and D20 rulings implemented; PC2 parity-candidate specs frozen. Phase 13 NOT READY: FOREXCOM:XAUUSD 1m V-1 exports, Dukascopy 2025-11-11, calibration, zero-lag check, frozen tolerances and final-run authorization outstanding.** Full tick history deferred (OQ-33); V-1 required before Phase 13. Phase 14 is
