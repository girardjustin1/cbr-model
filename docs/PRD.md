...
- **13 Tom ↔ Python parity gate: NOT READY** (checklist `docs/governance/phase13-readiness.md`, 20/28 items; parity-candidate specs `docs/strategy/parity-candidates/*-PC2.yaml` frozen, PC1 kept; parity library refuses the final run; V-1 state **V1_WAITING_FOR_OWNER_DATA**, supplied charts don't satisfy V-1: `reports/v1-export-inspection.md`). The engine must reproduce the taught examples before any performance research.
