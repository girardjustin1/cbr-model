...
**Doc:** CBR-PRD-001 · **Version:** v1.0 · status updated 2026-09-18 · **Status:** Active · **Current phase:** 14A.1 complete — Execution Simulator + Real-Data Integration (**14A accepted PASS WITH CONCERNS (D39)**, 14A.1 self-verdict **PASS**; next decision is OQ-24, the baseline data architecture; execution semantics frozen by D38; Phase 13 closed (D37) as RESEARCH-SUFFICIENT WITH DOCUMENTED FIDELITY CONCERNS with PC2, PC3 and PC4 parity runs all still **FAIL**; Phases 9-12 accepted with concerns) · **Instruments:** XAUUSD + DXY
**Repository:** `main` = `origin/main` at `5d05cce` (pushed); Phase 13 work on branch `cbrModel-work-091726-8pm` (PR #1 open from `cbr-work_091526-4pm`) · Full version: `docs/PRD.html`
...
| D19 | Phase 13 readiness rulings, implemented: prior-setup **existence** (OQ-36 C′); **5-second shift = CBR1H entry trigger**, variants A/B keep separate parent structures (OQ-39); condition window in tradable time (OQ-40, ASSUMPTION); Q−1 broken by Q (OQ-41); :30 timing at the shift, hard veto unresolved → diagnostic (OQ-42); HVCS runs into the shift, no indecision count (OQ-43, OQ-44 opened); parity protocol approved in principle; numeric tolerances not frozen; HTF signal/fill split; parity-candidate specs PC1 frozen. Record: `docs/governance/d19-phase13-readiness-ruling.md` |
| D20 | D19 follow-up: implementation accepted; setup formed = `raw_setup_armed` (non-recursive); CBR1H prior window ends at H.t0; no CBR15 Q−1 exception; OQ-44 HVCS continuity and OQ-45 CBR15 tradable-time window resolved as ASSUMPTIONS; **PC2** specs frozen (PC1 kept); V-1 ingestion prepared (`v1_ingest`); owner charts are FX:XAUUSD with 1m from 2026-09-08 only → DATA_LIMITATION. Record: `docs/governance/d20-d19-followup-ruling.md` |
| D21 | D20 accepted; V-1 preparation only. Dukascopy 2025-11-11 fetched and validated; both calibration days ready (`reports/v1-dukascopy-calibration-days.md`); calibration order and no-auto-freeze rule set for when FOREXCOM files arrive |
| D22 | Calibration tool completed (per-field residuals, missing minutes per source, p90, stability by day / hour / time block / volatility regime, zero-lag STOP). Supplied TradingView Gold exports are **FX:XAUUSD** (not FOREXCOM) with 1m from 2026-09-08 only → `V1_SYMBOL_MISMATCH`; higher timeframes kept as reference fixtures; TVC:DXY matches Tom's DXY feed label (OQ-26 evidence, unchanged). Assessment and options A/B/C: `docs/decisions/v1-tradingview-exports-assessment.md` |
| D23 | Tom chart data inspected: FOREXCOM:XAUUSD (probable identity) and TVC:DXY at 1m…1D; the intended Gold feed is now present, but its 1m history starts 2026-09-09 → `V1_TOM_DATA_INSUFFICIENT`; FOREXCOM 1h covers all five V-1 windows; no 1m DXY overlap with stored Dukascopy DXY. Assessment: `docs/decisions/v1-tom-chart-data-assessment.md` |
| D24 | **Phase 13 = Strategy Fidelity and Behavioral Parity.** Dukascopy stays canonical (sufficient, not identical to FOREXCOM). V-1 becomes supporting validation (historical FOREXCOM 1m unavailable). Gate A-G: causal/deterministic, PC2 implemented correctly, behavioral agreement on core CBR concepts in the three hourly examples, every mismatch classified, no unexplained implementation mismatch, feed differences documented, feed differences don't change signal membership at an unacceptable rate. Categorical verdicts, no match percentage. Pass = faithful enough to start execution and baseline testing (not profitability or edge). Criteria `docs/acceptance/phase13-acceptance-criteria.md`; proposed run `docs/governance/phase13-behavioral-parity-protocol.md` |
...
- **13 Strategy fidelity and behavioral parity: FAIL (D26).** PC2 identifies model family, direction and extension direction on 3/3 course examples and the core structural trigger on 0/3; feed differences are not the explanation. PC2 is permanently frozen as the failed candidate. **13R spec reconciliation** accepted (D26, D27). Rulings: D8 stays `HOUR_OPEN`; the `M1H-COND-04` prior-setup gate is not approved for PC3 (diagnostics retained); HVCS close-direction removal and causal end-time evaluation approved in principle. Upstream reviews delivered: **T3-1 type-3 sweep** (re-anchoring reproduces all three course triggers, 1.66× more raw events, negative controls clean), **extension activation** (Level 1: the overextension "can start around seven to 15 minutes in"), **weekend window** (no change; the direction test is the real issue, OQ-46). **PC3 created and implemented (D29)**: type-3 re-anchoring, PRE_EXTENSION/EXTENSION_ACTIVE with a minute-7 earliest activation and the Q1 qualifier, whole-extension pullback without a latch, structural HVCS evaluated at the shift, the hourly prior-setup gate demoted to diagnostics, and a directionless trending range classified as RANGE. PC2 is byte-identical and still frozen. **PC3 scored under CBR-PROT-013C v1.1 (D31), run CBR-RUN-013C-1: FAIL.** PC3 reproduces model family, direction and extension direction on 3/3 course examples and produces a D25-P3-equivalent 5s type-3 trigger on 3/3 — a gain over PC2's 0/3 — but no candidate survives to `ARMED_AT_TRIGGER`, so the core-trigger hard dimension is 0/3 under the frozen strict reading; the hour-level journal set is 0/3. The single blocking rule on all three course examples is `M1H-6A-1-HVCS-INTO-SHIFT` (measured conforming runs 3, 2 and 0 minutes against a CANON minimum of 4), classified UNRESOLVED_SPEC_AMBIGUITY because neither the run's anchor nor its counting convention was ever ruled on (OQ-48). 22 mismatches classified: 1 OWNER_BASELINE_CHOICE, 15 UNRESOLVED_SPEC_AMBIGUITY, 6 CANON_MISMATCH. Both runs hash identically (`718efebd…caef`). **D32: FAIL accepted; PC3 frozen, no PC4.** The failure diagnosis finds one blocking rule on all three positives (`M1H-6A-1-HVCS-INTO-SHIFT`), classified UNRESOLVED_SPEC_AMBIGUITY after eight counting readings were compared and none explained it (OQ-48); a separate implementation defect, PC3 silently dropping PC2's `hvcs.max_violations = 1` tolerance; an evaluation-instant pattern where PC3 evaluates extension-derived rules when the type 3 arms rather than at the entry (OQ-50); and a partly mis-scoped journal set — JM-2025-10-17 is an **IFS**, a middle-timeframe model with no class in PC2 or PC3 (OQ-49). Eight candidate changes are classified in D32 §6; none is applied. **D34: PC4 created (spec hash `62a2310e…`), scoring not authorized.** Three behavioural changes, each owner-approved and individually classified: the configured HVCS one-violation tolerance is applied again (F-1, IMPLEMENTATION_FIX), and the two rules that ask whether the active 15-minute candle has taken the previous candle's extreme are evaluated at the final 5-second shift with Q anchored there (F-9 CANON_CORRECTION, F-10 ASSUMPTION). The HVCS endpoint was **not** moved to the shift — that reading produces 3/3 and is preserved as a diagnostic only. PC2 and PC3 are provably byte-identical; 633 tests pass; the corrected parity set (manifest v2 `e12f3882…`) binds 4 entry-level and 1 hour-level case after the IFS and source-ambiguous journal rows were moved to narrative. CBR-PROT-013D is drafted and not run. **D35 / CBR-RUN-013D-1: FAIL.** PC4 scored under the approved protocol: **CX-LT1-1 and CX-TE1-1 are clean on all nine dimensions in the STRICT view** — the first time any candidate has produced an eligible signal on a course example — with the HVCS run measuring 6 and 4 minutes where PC3 measured 3 and 2. CX-LT3-2 still fails: at its shift the extension-extreme bar itself breaks the respected side, so the run measures 0 minutes, which is the disclosed consequence of the anchor D34 §3 deliberately kept, not a code defect. The negative control passed; the single hour-level case is 0/1, blocked upstream by the condition classifier (2 zig-zag legs, OQ-01). Both runs hash identically; 8 mismatches classified (1 OWNER_BASELINE_CHOICE, 7 UNRESOLVED_SPEC_AMBIGUITY, 0 IMPLEMENTATION_BUG). **D36: FAIL accepted; final source review.** CX-LT3-2 is classified a **DISCRETIONARY_EXCEPTION** on Tom's own words — he calls the condition one he is "not the biggest fan of" for reversals, says "it's not the best condition", accepts it with "I think this is good enough though", and never names an HVCS or a duration; the two examples PC4 reproduces cleanly are the ones his journal labels "textbook" and "new". The 20-minute extension rule is **guidance**, not a floor: every L1 statement is hedged and one calls "almost 20 minutes" a "really nice overextension" (OQ-53). There is **no HVCS source contradiction** — the requirement is inferred from variant A, not asserted by Tom. JM-2025-10-16 fails because the condition is classified once at the hour open, when 2 legs are confirmed, versus 4 by the journal's own minute (OQ-52). **No PC5 change is independently justified**, and the recommendation is **Path B** — PC4 sufficient with documented source ambiguity — which requires a new owner decision on the Phase 13 completion criterion. **D37 closes Phase 13.** The owner took Path B: PC4 is promoted to the frozen `CBR1H_BASELINE_V1` research specification with governance status **RESEARCH-SUFFICIENT WITH DOCUMENTED FIDELITY CONCERNS**. This is a promotion into baseline research, **not** a retroactive parity PASS — the scored runs stay FAIL for PC2, PC3 and PC4, and no verdict is rewritten. CX-LT3-2 is preserved as a DISCRETIONARY_REFERENCE_CASE, the ~20-minute rule is recorded as canonical guidance whose hard-floor implementation is left unchanged, and OQ-48, OQ-50, OQ-52 and OQ-53 stay open as future sensitivity experiments. Strategy logic is locked until the untouched baseline, enforced by `baseline_v1.verify()`. **Phase 14A is authorized**: signal generation and execution stay separate, no performance analysis until 14A acceptance, and ten execution assumptions (E-OQ-1…E-OQ-10) need owner rulings before fills can be implemented. **D38 freezes all ten execution assumptions and Phase 14A is built.** The stop is resolved at the entry fill from the anchor available causally through that fill and then frozen (OQ-28); there is **no** invented time exit, only the administrative `ROLLOVER_EXIT` and `DATASET_END_EXIT`; tick sequence decides stop-versus-target and anything it cannot resolve returns `EXECUTION_UNPROVABLE_INTRABAR` rather than a favourable convention; entries and stops fill at the first executable quote (so gaps hurt) while targets never claim improvement; additional slippage and commission are 0 as declared ASSUMPTIONS; one open CBR1H position per instrument, with siblings cancelled at the winner's fill; and no spread, quote or outcome is ever synthesised. Seven modules under `src/cbr/execution/`, 35 synthetic tests including a hand-computed worked example, an import guard that forbids any CBR rule module inside execution, 696 tests passing. No historical performance was run. **D39 accepts 14A as PASS WITH CONCERNS — the concern being integration, not semantics — and 14A.1 wires the simulator to real data.** Execution quotes now come from stored Dukascopy bid/ask ticks with per-file SHA-256 provenance; a real hour loads 26,393 ticks. Interval classification **translates** the existing Phase 9 criterion rather than inventing a second gap detector, so scheduled closures and sparse quoting are never mistaken for missing data. The DST-aware rollover calendar is reused and proven in both EST and EDT on real dates. Twenty integration tests inject **synthetic** signals into **real** quotes, so the mechanics are proven without exposing strategy performance — no CBR signal outcome was replayed and no win rate, expectancy or P&L exists. 716 tests passing. The OQ-24 package now recommends **full Dukascopy tick history** (≈5.6 GB for 2018-2024) on signal-membership grounds: a staged screen would have to be proven incapable of excluding a valid signal, and that proof needs the full data anyway. **D33 evidence resolution:** two dated Level-1 HVCS examples outside the parity set (2025-10-23 and 2025-10-29) each contain exactly one structural violation; PC3 measures 1 conforming minute on both, and 9 and 10 with the configured `max_violations = 1` restored — independent confirmation of F-1, which the owner had already approved. OQ-50 is answered from the master slide's step order (the take is a precondition of the *entry*, and the type-3 arm has no counterpart in the course), and the MTF taxonomy review finds three middle-timeframe models where the engine implements two, which removes JM-2025-10-17 (IFS) and JM-2025-10-29 (source ambiguity) from any future CBR1H machine set. PC4 is **not ready**: the HVCS anchor, the OQ-50 scope, and the corrected parity set need owner rulings first. Phase 14 not authorized (D24; gate `docs/acceptance/phase13-acceptance-criteria.md`; checklist `docs/governance/phase13-readiness.md`: mandatory 20/28, V-1 supporting 1/8). Run protocol CBR-PROT-013B **approved (D25)** with P-1…P-8 settled and a pre-declared dimension-by-dimension acceptance standard: behavioral dimensions on CX-LT1-1, CX-TE1-1, CX-LT3-2 with frozen PC2 on Dukascopy; FOREXCOM 1h/4h/1D higher-timeframe fidelity; recent FOREXCOM-vs-Dukascopy 1m feed study. V-1 (historical FOREXCOM 1m) is supporting only: unavailable because of TradingView history limits.
...
| G2 Faithful implementation | 13 | Strategy fidelity and behavioral parity (D24, CBR-ACC-013 gate A-G): behavioral agreement with the taught examples on Dukascopy spot; every mismatch classified; feed differences documented; V-1 supporting only | Owner |
...
| OQ-25 | Canonical OHLC source for extremes | **Resolved D16:** tick mid STRUCTURE, tick bid/ask EXECUTION; V-1 supporting validation since D24 |
| OQ-34 | CBR15 hourly veto depends on the CBR1H engine | Implemented split (D19-15): `htf_signal_state` is the rule; `htf_fill_state` `EXECUTION_DEPENDENT`, never rejects; CBR15 not fully eligible until 14A |
| OQ-35 | Stop anchor | **Resolved D17-2:** most adverse STRUCTURE extension extreme through activation/fill; separate fields |
| OQ-36 | COND-03 "prior working setup" meaning | **Resolved D19-1 (C′):** prior setup must have FORMED in the lookback; played-out status diagnostic only |
...
| OQ-39 | CBR1H entry trigger: spec 1m HILO / fractal vs Tom's 5s shift in all three examples | **Resolved D19-2:** 5s shift is the canonical trigger; variants A (HVCS) and B (1m T3 → 50%) separate |
| OQ-40 | Condition window across weekend closures (CX-LT3-2 → UNDEFINED) | **Assumption D19-3:** tradable-time window for CBR1H; clock/tradable/missing minutes recorded. CBR15 basis → OQ-45 |
| OQ-41…43 | CBR1H readings: prev-15m reference, q30 veto timing, HVCS end bar | **Resolved D19-4/5/6:** Q−1 broken by Q; :30 evaluated at the shift (hard veto still open, diagnostic); HVCS into the shift (indecision limit OQ-44) |
| OQ-44 / OQ-45 | HVCS continuity; CBR15 condition-window basis | **Resolved D20 as ASSUMPTIONS:** structural continuity with diagnostics only; tradable-market time for CBR15 |
