# Phase 13D PC4 Parity Protocol (PROPOSAL)

**Doc:** CBR-PROT-013D · **Version:** v1.0 **PROPOSED** · **Date:** 2026-09-17 · **Ruling:** D34 §18
**Status:** NOT APPROVED and NOT RUN. Drafted under D34 §18, which authorizes preparing this protocol and forbids
executing it. PC4 scoring is not authorized.
...
Hard dimensions, unchanged from D31: **model family, direction, extension direction, core structural trigger**.
...
CANDIDATE_SELECTION_MISMATCH. A rule whose failing quantity is a carried ASSUMPTION is not a canon mismatch. Every
mismatch is classified; an unclassified mismatch is a FAIL.
...
## 7. Acceptance rules (for owner approval)

| Id | Rule |
|---|---|
| **D-1** | **ENTRY_LEVEL positives:** all three must match hard dimensions 1, 2, 4 and 8 unless a pre-existing approved mismatch class applies |
| **D-2** | **Remaining hard dimensions** (3, 5, 6, 7, 9): each mismatch classified and reported; an unclassified mismatch is a FAIL |
| **D-3** | **HOUR_LEVEL:** `HOUR_MATCH` only when every source-supported field agrees. With one binding hour-level case the threshold is **1/1 for a clean PASS; 0/1 caps the verdict at PASS WITH CONCERNS and does not by itself fail the run** — a single case is too thin to carry a FAIL on its own, and this is the one rule that differs from D31, which had three. **Owner decision required.** |
...
## 12. Owner decisions required before any run

| Id | Decision |
|---|---|
| E-1 | Approve this protocol and the §2 dimensions |
| E-2 | Set the D-3 hour-level threshold now that only one hour-level case binds |
| E-3 | Confirm the corrected parity set (`e12f3882…`) as the scoring set |
| E-4 | Confirm CBR15 stays out of scope |
| E-5 | Authorize the scored run — the only step that lets PC4 see the parity cases |
